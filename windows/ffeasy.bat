...
Inbox
	x
Adam Quinton <quintonish@gmail.com>
...
:lowleveluser
title BUSY
bin\ffmpeg.exe -loglevel error -i %INPUTEE% -r %FPS% -q %QUAL% -vf scale=%WIDTH%:-1 -acodec mp3 -b:a %ABITRATE% -vcodec libxvid %OUTPUTEE%.avi
title DONE
goto prompt
...
:androidCharmCon
TITLE BUSY
bin\ffmpeg -i %input% -vcodec libx264 -preset medium -vprofile baseline -vf scale=320:-1 -b:v 384k -acodec aac -ac 2 -ab 128k -ar 44100 -strict experimental %output%
TITLE DONE
goto prompt
...
for %%a in ("*.*") do (
   ...
   ..\bin\ffmpeg -i "%%a" -vcodec libx264 -preset medium -vprofile baseline -vf scale=320:-1 -b:v 384k -acodec aac -ac 2 -ab 128k -ar 44100 -strict experimental "..\%OUTPUT%\%%~na.mp4"
   echo "%%a" >> ..\bin\log.txt
   set /A CFILE+=1
)
set /A CFILE-=1
...
:final
Attachments area
Preview attachment ffeasy.txt
[Text]
	
Click here to Reply or Forward
7.06 GB (47%) of 15 GB used
Manage
Terms - Privacy
